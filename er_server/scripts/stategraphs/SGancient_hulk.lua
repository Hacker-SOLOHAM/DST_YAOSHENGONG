...
{    
    --ActionHandler(ACTIONS.SPECIAL_ACTION, nil), --特别动作
}
...
local function ShakeIfClose(inst)
    ...
    if player then
		...
		ShakeAllCameras(CAMERASHAKE.FULL, 0.7, 0.02, 3, inst, 40)
		--player:ShakeCamera(CAMERASHAKE.FULL, 0.7, 0.02, 3, 40)
    end
end
...
local function ShakeIfClose_Footstep(inst)
    ...
    if player then
		...
		--player:ShakeCamera(CAMERASHAKE.FULL, 0.35, 0.02, 1.25, 40)
		ShakeAllCameras(CAMERASHAKE.FULL, 0.35, 0.02, 1.25, inst, 40)
    end
end
...
----------------------SPIN--------------------
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.components.combat.playerdamagepercent = 2 --对玩家的伤害比
        end,            
