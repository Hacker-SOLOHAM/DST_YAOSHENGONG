...
local  colourrate = { 1.05,1.1,1.15,1.2,1.25,1.3, }	--彩色宝石增加驯服度的系数

local function DoDeltaDomestication(self, delta)	---驯服度改变的时候 判定一次数据
	if self.inst.components.rideable and self.inst.components.rideable.saddle ~= nil then
		local saddle = self.inst.components.rideable.saddle
		if saddle.components.rgwuqinew then
			local black = saddle.components.rgwuqinew.black
			if black ~= 0 then
				if delta > 0 then
					delta = delta * colourrate[black]
				elseif delta < 0 and black == 6 then
					delta = 0
				end
			end
		end
