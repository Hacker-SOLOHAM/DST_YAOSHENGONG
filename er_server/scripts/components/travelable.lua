...
function Travelable:Travel(traveller, index)
    ...
    if traveller and destination then
        ...
        if destination and destination.components.travelable then
            ...
            information = string.format("前往: %s (%d/%d)\n饥饿消耗: %.0f\n精神消耗: %.1f",description, self.site, self.totalsites,cost_hunger, cost_sanity)
            if comment then
            ...
            self.traveltask = self.inst:DoTaskInTime(4, function()
                self.traveltask = nil
